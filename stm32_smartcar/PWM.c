...
void PWM_Init(void)
{
      ...
      TIM_OCInitStructure.TIM_Pulse = 0; //初始捕获比较寄存器中的脉冲值为0
      ...
      TIM_OCInitStructure.TIM_Pulse = 0;//加载到比较寄存器中的初始值为0；
      TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;//极性为高极性
       //TIM4_CH2初始化,OC2
