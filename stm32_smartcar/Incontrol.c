...
/*******************************************************************************
* 函 数 名         : IRremote_Init//相当于进入了外部中断
* 函数功能		   : 红外端口初始化函数时钟  端口及外部中断初始化 
* 输    入         : 无
* 输    出         : 无
*******************************************************************************/
void IRremote_Init(void)//外部中断配置，RCC>>GPIO>>AFIO>>EXTI>>NVIC
{
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinSource8); //选择GPIO管脚用作外部中断线路//AFIO配置
...
* 函数功能		   : 高电平持续时间将记录的时间   保存在t中返回其中一次大约20us 
* 输    入         : 无
* 输    出         : t
*******************************************************************************/
